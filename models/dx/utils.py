...
def extract_pairs(file_path, sample_count=3):
    with open(file_path, 'r', encoding='utf-8') as f:
        all_lines = f.readlines()

    chosen_lines = random.sample(all_lines, sample_count)
    prompts, results = [], []

    for selected_line in chosen_lines:
        data_item = json.loads(selected_line)
        prompt = data_item.get('query')
        result = data_item.get('response')
